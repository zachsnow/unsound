...
export function build$compile(in$: CompileOps): void {
  const $ = in$ as CoreCompileOps;
  ...
  $.compileExpr = (expr) => {
    switch (expr.type) {
      case "LetExpr":
        return $.compileLet(expr);
      case "LambdaExpr":
        return $.compileLambda(expr);
      case "AppExpr":
        return $.compileApp(expr);
      case "IfExpr":
  ...
  $.compileObject = (expr) => {
    return ir.$(
      "object",
      ir.object(
        expr.properties.map((p) => ir.prop(p.key, $.compileExpr(p.value)))
      )
    );
  };
  ...
  $.compileIndex = (expr) => {
    return ir.$("index", $.compileExpr(expr.object), $.compileExpr(expr.key));
  };
...
export const $compile: CoreCompileOps = fix<CoreCompileOps>(build$compile as Builder<CoreCompileOps>)
