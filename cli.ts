...
async function generateStandalone(source: string, extNames: string[], noCore: boolean, printResult: boolean = false): Promise<string> {
  ...
  // For standalone/binary output, import extensions directly so bun can bundle them
  // (rather than using loadExtension which needs runtime file access)
  const coreImport = noCore ? '' : `import coreExtension from '${path.resolve(sourceDir, 'extensions/core.ts')}';`;
  const coreApply = noCore ? '' : 'applyExtension(lang, coreExtension);';
